...
### !!!!!!!NOTA!!!!!!! Ten cuidado si tienes ya generados ya ciertas partes del texto que no se llego a traducir (ejemplo: por nombre del protagonista) no Reemplaces el archivo _AutoGeneratedTranslations.txt

## El mod ha modificado mi nombre y tambien afecta el chat general del juego ¿que puedo hacer?
El mod usa un auto traductor como mencione en el proyecto agarra las traducciones automaticas por si hay alguna traducción que no tenga a la mano (si se fijan el nombre de mi protagonista se llama Nagisa Kamisiro el default) por lo que el texto de algunos dialogos no lo tomara por no tener el nombre tal cual como esta escrito en la traducción manual, por lo que deje activo esa opción.
